# make efficiency and performance plots comparing muon ptcone20_final, etcone20, etcone20_final and topoetcone20_final and same for 0.4 dr cone
...
output_list = glob.glob('output_isol.root')

outfile = TFile('output_muon_topoiso.root', 'RECREATE')

rel_cut_values = numpy.linspace(0., 5., 100)
abs_cut_values = numpy.linspace(-3000, 50000, 200)
...
for obs in observables:
  ...
  histo_obs[obs] = TH1F('histo_%s' % obs, 'histo_%s' % obs, 100, min(proper_cut_values), max(proper_cut_values))
  histo_obs_vs_bcid[obs] = TH2F('histo_%s_vs_bcid' % obs, 'histo_%s_vs_bcid' % obs, 3000, 0, 3000, 100, min(proper_cut_values), max(proper_cut_values))
  histo_obs_vs_nvx[obs] = TH2F('histo_%s_vs_nvx' % obs, 'histo_%s_vs_nvx' % obs, 50, 0, 50, 100, min(proper_cut_values), max(proper_cut_values))
  histo_obs_vs_averageIntPerXing[obs] = TH2F('histo_%s_vs_averageIntPerXing' % obs, 'histo_%s_vs_averageIntPerXing' % obs, 50, 0, 50, 100, min(proper_cut_values), max(proper_cut_values))
  histo_obs_vs_actualIntPerXing[obs] = TH2F('histo_%s_vs_actualIntPerXing' % obs, 'histo_%s_vs_actualIntPerXing' % obs, 50, 0, 50, 100, min(proper_cut_values), max(proper_cut_values))
